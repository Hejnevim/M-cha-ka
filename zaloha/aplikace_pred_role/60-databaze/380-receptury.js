...
function Recipes({ recipes, setRecipes, guardDelete, dbFiltr, setDbFiltr, technologie, dbTech, sita,
                   materialy, onUlozitCeny, cenyStav, mostOk }) {
  ...
  const filtered = useMemo(() => {
    ...
    const zaklad = podleTechnologie(podleDatabaze(recipes, dbFiltr), technologie, dbTech);
    if (!s) return zaklad;
    return zaklad.filter((r) => (r.name + " " + r.series + " " + r.type).toLowerCase().includes(s));
  }, [q, recipes, dbFiltr, technologie, dbTech]);
  ...
  const save = (r) => {
    setRecipes((prev) => {
      ...
      if (i === -1) return prev.concat([r]);
      const c = prev.slice(); c[i] = r; return c;
    });
    setEdit(null);
  };
