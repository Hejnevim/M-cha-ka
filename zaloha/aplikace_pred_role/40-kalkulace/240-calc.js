...
function Calc({ products, recipes, setRecipes, links, setLinks, spec, onSpecUsed, onUpravitSpec,
                ...
                onDoFronty,
                technologie, dbTech, sita, koef, pigmenty, guardDelete }) {
  ...
  const pantoneAll = podleDatabaze(recepturyTech, dbFiltr).filter((r) => r.type !== "Custom");
  ...
  const zakladAll = useMemo(() => recepturyTech.filter((r) => r.type !== "Custom"), [recepturyTech]);
...
<style>
  ...
  <div class="box g2">
    ...
    <div><div class="lbl">Barva / řada</div><div class="val">${esc(recipe.name)}${recipe.series ? " · " + esc(recipe.series) : ""} · hustota ${fmt(n(recipe.density, 1), 2)} g/ml${jeAdHoc ? " · <b>neuložená receptura</b>" : ""}</div></div>
            ...
            <div className="frow c2 recept-pulky">
              <div>
                ...
                <${FiltrDatabaze} recipes=${zakladAll} hodnota=${dbFiltr} setHodnota=${setDbFiltr}
                  nadpis=${false} vzdy=${true} vyber=${true} />
              ...
              <div>
                ...
                <div style=${{ marginBottom: 10 }}>
                  <select value=${custFiltr} onChange=${(e) => setCustFiltr(e.target.value)}>
                    <option value="">Všechny řady (${fmt(customList.length, 0)})</option>
                ...
                <div>
                  <select value=${customVidet.some((x) => x.r.id === recId) ? recId : ""}
                    ...
                    ${customVidet.map(({ r, presna, volna }) => html`<option key=${r.id} value=${r.id}>${
                      r.name}${presna ? " ✓ tato kombinace" : (volna ? " · bez vazby" : "")}</option>`)}
                  </select>
                  ${customVybrany && html`
