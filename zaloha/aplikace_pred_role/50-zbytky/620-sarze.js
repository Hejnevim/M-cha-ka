...
const klicMaterialu = (s) => String(s || "").trim().toLowerCase();
...
/* Šarže se v souboru pozná dvojicí materiál + kód. Samotný kód nestačí:
   ...
   označení šarže. */
const klicSarze = (s) => klicMaterialu(s && s.material) + " "
...
function otevrenaSarze(sarze, material) {
  const k = klicMaterialu(material);
  if (!k) return null;
  let nej = null;
  for (const s of (sarze || [])) {
    if (s.stav !== "otevrena" || klicMaterialu(s.material) !== k) continue;
...
function otevritKonev(sarze, udaje) {
  ...
  const k = klicMaterialu(udaje && udaje.material);
  const zavrene = (sarze || []).map((s) =>
    (s.stav === "otevrena" && klicMaterialu(s.material) === k)
...
function dojetaKonev(sarze, kodSarze, material) {
  const nyni = Date.now();
  const k = klicMaterialu(material);
  return (sarze || []).map((s) => (s.kod === kodSarze && klicMaterialu(s.material) === k)
...
function prehledKonvi(sarze) {
  const podle = new Map();
  for (const s of (sarze || [])) {
    const k = klicMaterialu(s.material);
