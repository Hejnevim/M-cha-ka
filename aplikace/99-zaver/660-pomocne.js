...
/* `dbMat` + `matProduktu` přidají k řadám vhodnost na materiál produktu.
   ...
   z čeho soudit — typ bez vyplněných materiálů zůstane bez značky. */
function FiltrDatabaze({ recipes, hodnota, setHodnota, popis, tech, skryto, nadpis, vzdy, vyber,
  ...
  /* Uklidit po sobě ale smí jen ten filtr, na který je právě vidět. Filtr je
     jeden společný pro celou aplikaci, kdežto kalkulace se při odskočení jinam
     neruší — jen schová — a dál si zužuje seznam podle technologie své polohy
     potisku. Dokud hlídání platilo i pro schovanou kartu, vrátila volbu
     udělanou v Recepturách zpátky na „vše“ dřív, než ji obrazovka stihla
     ukázat: databáze patřící k jiné technologii než ta rozdělaná nešla zapnout
     vůbec a klik na ni vypadal, že nic nedělá. */
  useEffect(() => {
    if (aktivni === false) return;
    if (!hodnota) return;
    const je = zdroje.some((z) => (z.zdroj || "@vlastni") === hodnota);
    if (!je) setHodnota("");
  }, [hodnota, zdroje, aktivni]);
