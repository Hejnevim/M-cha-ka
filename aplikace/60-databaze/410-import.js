...
function csvToRecipes(text, zdroj) {
  ...
  const ci = {
    ...
    // schválení vlastní receptury; chybějící sloupec = schválená (viz 225-role.js)
    schvaleni: idx(/^(schvaleni|schv.len.|stav.schvalen|approval|approved)/),
    schvalil: idx(/^(schvalil|schv.lil|approved_by)/),
    schvalenoKdy: idx(/^(schvaleno.kdy|schv.leno.kdy|approved_at)/),
  ...
  for (const r of rows.slice(1)) {
    ...
    if (!map.has(name)) {
      map.set(name, {
        ...
        schvalenoKdy: ci.schvalenoKdy >= 0 ? n(r[ci.schvalenoKdy]) : 0,
...
/* Receptura, jejíž soubor už na disku není — po přejmenování databáze. Poznat
   ji jinde než podle názvu a řady zapsané v CSV nejde: týž „PANTONE 485 C“ je
   v Ferro i v Printcolor, ale řada u něj stojí pokaždé jiná. */
...
function sloucReceptury(prev, nove, adopce, zijiciSoubory) {
  ...
  /* Přejmenovaný soubor databáze. Receptury z něj zůstaly v prohlížeči zapsané
     ...
     se táž databáze načetla podruhé a v nabídce by stály dva typy barvy vedle
     sebe: nový ze souboru a starý, ke kterému už soubor nevede. Sirotek si
     nese id, a na id visí vazby na produkt a polohu i to, co k receptuře
     nastavil technolog. */
  const kSirotkum = new Map();
  if (zijiciSoubory) {
    for (const r of prev) {
      if (!r.zdroj || zijiciSoubory.has(r.zdroj)) continue;
      kSirotkum.set(klicSirotka(r), r);
    }
  }
  ...
  let pridano = 0, obnoveno = 0, prevzato = 0;
  for (const r of nove) {
    ...
    let jeSirotek = false;
    if (!stary && r.zdroj && kSirotkum.size) {
      const sirotek = kSirotkum.get(klicSirotka(r));
      if (sirotek) {
        stary = sirotek;
        jeSirotek = true;
        mapa.delete(klicReceptury(sirotek));
        kSirotkum.delete(klicSirotka(r));
      }
    }
    if (stary) {
      if (jeSirotek) prevzato++; else obnoveno++;
      ...
      mapa.set(klic, Object.assign({}, r, {
        ...
        /* Razítko schválení taky. Databáze od dodavatele sloupce nemá a přepsat
           je prázdnem by znamenalo, že se čekající receptura tiše stane
           schválenou — právě to, čemu má schvalování bránit. */
        schvaleni: r.schvaleni || stary.schvaleni || "",
        schvalil: r.schvalil || stary.schvalil || "",
        schvalenoKdy: n(r.schvalenoKdy) || n(stary.schvalenoKdy) || 0,
