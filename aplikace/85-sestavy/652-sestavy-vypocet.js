...
/* Všechno, co se kdy namíchalo, v jedné řadě: dávky i kelímky, které dávku
   ...
   `recipes` je nepovinné a slouží skladu surovin: s nimi si každý záznam
   nese i to, z čeho se míchal (složení, tužidlo, gramy vzaté ze zbytku).
   Rozpad na složky se dělá tady, a ne ve druhém průchodu daty, protože právě
   tady se rozhoduje, která směs se počítá a která je jen zapsaná podruhé —
   dvě podobná pravidla vedle sebe by se dřív nebo později rozešla. */
function michaniZaznamy(davky, zbytky, recipes) {
  ...
  /* Receptura se páruje názvem barvy, ne id — to se při každém načtení
     databáze přiděluje znovu a po restartu by ukazovalo jinam. */
  const podleNazvu = new Map();
  for (const r of (recipes || [])) {
    const k = nazevOdstinu(r.name).toLowerCase();
    if (!podleNazvu.has(k)) podleNazvu.set(k, r);
  }
  const receptura = (jm) => podleNazvu.get(nazevOdstinu(jm).toLowerCase()) || null;
  ...
  for (const d of (davky || [])) {
    ...
    const jmeno = nazevOdstinu(d.nazev || (k ? k.nazev : ""));
    const rec = receptura(jmeno);
    out.push({
      kdy: kdy, kod: d.kod, zdroj: "davka",
      nazev: jmeno,
      gramu: gramu, tech: d.tech || (k ? k.tech : ""), zakazka: d.zakazka || "",
      hex: (k && k.hex) || "",
      /* Rozpad směsi na složky. Kelímek si nese složení tak, jak se doopravdy
         míchalo, takže má přednost před recepturou — ta se mezitím mohla
         přeladit a odečetlo by se ze skladu něco jiného, než co se navážilo. */
      bazeG: n(d.bazeG), tuzidloG: n(d.tuzidloG),
      slozeni: (k && (k.slozeni || []).length) ? k.slozeni
        : (rec ? podilySlozeni(rec.components) : []),
      tuzidloNazev: (rec && rec.tuzidloNazev) || "",
      zbytekG: k ? n(k.zbytekG) : 0, zbytekKod: (k && k.zbytekKod) || "",
  ...
  for (const z of (zbytky || [])) {
    ...
    const rec = receptura(z.nazev);
    /* Kelímek bez dávky si tužidlo nenese v gramech, jen příznakem a poměrem
       — dávku zakládá jen dvousložková barva. Dopočítá se stejným vzorcem
       jako u váhy: podíl z váhy BÁZE, ne z hotové směsi. */
    const tuz = z.tuzidlo ? gramu * naPodil(z.pomerTuzidla, POMER_TUZIDLA_VYCHOZI) : 0;
    out.push({
      ...
      bazeG: gramu, tuzidloG: tuz,
      slozeni: (z.slozeni || []).length ? z.slozeni
        : (rec ? podilySlozeni(rec.components) : []),
      tuzidloNazev: (rec && rec.tuzidloNazev) || "",
      zbytekG: n(z.zbytekG), zbytekKod: z.zbytekKod || "",
    });
  }
...
function prehledSestav({ davky, zbytky, materialy, mesicu, ted }) {
  ...
  /* Prázdné měsíce na začátku okna se useknou. Mezera uvnitř řady je údaj
     — tehdy se nemíchalo —, kdežto měsíce před první zapsanou dávkou říkají
     jen to, že evidence ještě neběžela. Kolik se jich useklo, se řekne
     nahlas; jinak by sestava tvrdila dvanáct měsíců a ukazovala čtyři. */
  const prvni = mesice.findIndex((m) => m.pocet > 0);
  const zkraceno = prvni > 0 ? prvni : 0;
  if (zkraceno) mesice.splice(0, zkraceno);
  ...
  return {
    od: od, mesice: mesice, zkraceno: zkraceno, odstiny: odstiny,
