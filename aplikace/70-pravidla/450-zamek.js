...
function zapisCenyDoCsv(text, zmeny) {
  ...
  // těkavé látky a bezpečnostní list jdou toutéž cestou jako cena — je to
  ...
  // uvozovek by rozsypal sloupce celého řádku
  const bunka = (v) => /[;"]/.test(String(v)) ? '"' + String(v).replace(/"/g, '""') + '"' : String(v);
  ...
  for (let i = 1; i < radky.length; i++) {
    ...
    b[iMena] = z.mena || "";
    b[iJed] = z.jednotka || "";
    // jen když změna ty údaje nese — starší volání by jinak tiše smazalo,
...
/* Zapíše zásoby do tabulky materiálů a nechá všechno ostatní být — týmž
   způsobem jako ceny a ze stejného důvodu: pigmenty.csv je pro dílnu čitelný
   dokument s vysvětlivkami a odstíny naladěnými podle vzorníku.

   Píše se tady zvlášť, a ne jedním zápisem s cenami, protože ceník a sklad
   mění dva různí lidé ve dvou různých chvílích: cena přijde od dodavatele
   jednou za čas, zásoba se přepočítá při inventuře. Kdyby se ukládalo obojí
   naráz, přepsala by inventura ceny, které mezitím někdo opravil na druhém
   počítači.

   Do `zasoba_kdy` se zapisuje čas inventury, ne čas uložení souboru. Zůstatek
   se od něj počítá dopředu spotřebou z evidence, takže posunuté datum by tiše
   zahodilo dávky namíchané mezitím. */
function zapisSkladDoCsv(text, zmeny) {
  const zdroj = String(text || "").replace(/^﻿/, "");
  const radky = zdroj.split(/\r?\n/);
  let hlavicka = rozdelRadek(radky[0] || "").map((h) => h.trim());
  const najdi = (re) => hlavicka.findIndex((h) => re.test(h.toLowerCase()));
  ...
  const iDruh = najdi(/^(druh|typ)/);
  if (iNazev < 0 || iDruh < 0)
    throw new Error("Tabulka materiálů nemá sloupce druh a nazev — zásoby není kam zapsat.");
  ...
  const iZas = dopln("zasoba", /^(zasoba|z.soba|sklad|stock)/);
  const iMin = dopln("min_zasoba", /^(min_zasoba|min.?z.soba|minimum|min_stock)/);
  const iKdy = dopln("zasoba_kdy", /^(zasoba_kdy|z.soba_kdy|inventura)/);
  const iBal = dopln("baleni", /^(baleni|balen.|package|pack)/);
  radky[0] = hlavicka.join(";");
  const nejdal = Math.max(iZas, iMin, iKdy, iBal);

  const zbyva = new Map();
  for (const z of (zmeny || [])) zbyva.set(String(z.nazev).trim().toLowerCase(), z);

  /* Mění se jen ta pole, která změna doopravdy nese. Kdo přepsal jen minimum,
     nesmí tím shodit datum poslední inventury na prázdno. */
  const naplnit = (b, z) => {
    if (z.zasoba !== undefined)
      b[iZas] = z.zasoba == null || z.zasoba === "" ? "" : cislo(z.zasoba, 3);
    if (z.minZasoba !== undefined)
      b[iMin] = z.minZasoba == null || z.minZasoba === "" ? "" : cislo(z.minZasoba, 3);
    if (z.zasobaKdy !== undefined)
      b[iKdy] = n(z.zasobaKdy) > 0 ? cislo(z.zasobaKdy, 0) : "";
    if (z.baleni !== undefined)
      b[iBal] = z.baleni == null || z.baleni === "" ? "" : cislo(z.baleni, 3);
  };
  ...
  for (let i = 1; i < radky.length; i++) {
    ...
    const z = zbyva.get(klic);
    if (!z) continue;
    while (b.length <= nejdal) b.push("");
    naplnit(b, z);
    radky[i] = b.join(";");
    zbyva.delete(klic);
  }

  /* Složka, která v tabulce ještě není — přibude i s druhem, ať se pozná,
     co to je; bez druhu by ji ceník ani sklad při dalším čtení nenačetly. */
  for (const z of zbyva.values()) {
    if (!String(z.nazev || "").trim()) continue;
    const b = [];
    for (let k = 0; k < hlavicka.length; k++) b.push("");
